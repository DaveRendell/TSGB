...
export function MonochromePalettePicker({ pictureProcessor }: Props) {
  const [monochromePalette0, setMonochromePalete0] = React.useState(
    arrayToColour(pictureProcessor.scanlineRenderer.colours[0]),
  )
  const [monochromePalette1, setMonochromePalete1] = React.useState(
    arrayToColour(pictureProcessor.scanlineRenderer.colours[1]),
  )
  const [monochromePalette2, setMonochromePalete2] = React.useState(
    arrayToColour(pictureProcessor.scanlineRenderer.colours[2]),
  )
  const [monochromePalette3, setMonochromePalete3] = React.useState(
    arrayToColour(pictureProcessor.scanlineRenderer.colours[3]),
  )
  const [chosenPresetId, setChosenPresetId] = React.useState(-1)

  const updatePalette = (id: number, colour: number[]) => {
    pictureProcessor.scanlineRenderer.colours.splice(id, 1, colour)
    window.localStorage.setItem(
      "monochromePalette",
      JSON.stringify(pictureProcessor.scanlineRenderer.colours),
    )
    switch (id) {
      case 0:
